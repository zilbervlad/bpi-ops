...
class UploadedPhoto(db.Model):
    ...
    source_type = db.Column(db.String(50), nullable=False, index=True, default="svr")
...
# === BPI ACADEMY MIT STS MODELS START ===
class MITProfile(db.Model):
    ...
    mit_user = db.relationship(
        "User",
        foreign_keys=[user_id],
        backref=db.backref("academy_mit_profiles", lazy=True),
    )

    coach_user = db.relationship(
        "User",
        foreign_keys=[coach_user_id],
        backref=db.backref("academy_coached_mits", lazy=True),
    )
...
class MITLevelProgress(db.Model):
    ...
    verified_by_user = db.relationship(
        ...
        backref=db.backref("academy_verified_mit_items", lazy=True),
    )
...
class MITReview(db.Model):
    ...
    reviewer_user = db.relationship(
        ...
        backref=db.backref("academy_mit_reviews_written", lazy=True),
    )
...
class MITActionPlanItem(db.Model):
    ...
    owner_user = db.relationship(
        ...
        backref=db.backref("academy_mit_action_items_owned", lazy=True),
    )
...
class MITPromotion(db.Model):
    ...
    approved_by_user = db.relationship(
        ...
        backref=db.backref("academy_mit_promotions_approved", lazy=True),
    )
...
class MITTask(db.Model):
    ...
    assigned_by_user = db.relationship(
        ...
        backref=db.backref("academy_mit_tasks_assigned", lazy=True),
    )
...
class MITBinderSubmission(db.Model):
    ...
    approved_by_user = db.relationship(
        "User",
        foreign_keys=[approved_by_user_id],
        backref=db.backref("academy_binder_submissions_approved", lazy=True),
    )

    sheet_approved_by_user = db.relationship(
        "User",
        foreign_keys=[sheet_approved_by_user_id],
        backref=db.backref("academy_binder_sheet_approvals", lazy=True),
    )
...
# === BPI ACADEMY MIT STS MODELS END ===
